...
- **Full commit detail panel** — author/committer split (so you can actually tell a rebase or cherry-pick apart from the original commit), GPG signature status, diffstat, a file tree, and a syntax-highlighted diff that can expand to a full-page view — with a resizable file list and full paths on hover — for reading a real changeset comfortably.
